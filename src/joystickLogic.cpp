...
void joystickLogic(Redis &redis, Thread &workerThread)
{
  ...
  *timerWatchdog >> [&](const TimerMsg &)
  ...
  redis.subscriber<int>("src/joystick/button/0") >>
      *new LambdaFlow<int, int32_t>([&](int32_t &out, const int &in)
                                     { INFO(
                                           " button 0 : %s", in ? "true" : "false");
                                           if (in) {
                                             out = 0;
                                             return true;
                                           } return false; }) >>
      redis.publisher<int32_t>("dst/raspi/gpio9/value");
  ...
  redis.subscriber<int>("src/joystick/button/0") >>
      *new LambdaFlow<int, int32_t>([&](int32_t &out, const int &in)
        ...
        return false; }) >>
      redis.publisher<int32_t>("dst/raspi/gpio15/value");

  redis.subscriber<int>("src/joystick/button/1") >>
      *new LambdaFlow<int, int32_t>([&](int32_t &out, const int &in)
                                    {
    if (in)
    {
